...
function parse (
  ...
  postTransforms = pluckModuleFunction(options.modules, 'postTransforms');
  ...
  function closeElement (element) {
    ...
    // check pre state
    // 检查 v-pre 和 pre 标签，将在 options.start 中设置为 true的属性恢复成原始的 false，便于下一个元素处理
    if (element.pre) {
      inVPre = false;
    }
  ...
  parseHTML(template, {
    ...
    // 例子：tag='div' attrs=[{name: 'id', value: 'test_id', start:5,end:7}, {name: '@click', value: 'onClick',start,end}] unary：Boolean，是不是一元标签
    start: function start (tag, attrs, unary, start$1, end) {
      ...
      // 创建 ASTElement，即 element 就是一个元素节点的 AST 对象
      // function createASTElement (tag, attrs,  parent) {
      //   return {
      //     type: 1,
      //     tag: tag,
      //     attrsList: attrs, // [{name: 'id', value: 'test_id',start,end}, {name: '@click', value: 'onClick',start,end}]
      //     attrsMap: makeAttrsMap(attrs), // 校验是属性是否是重复定义,返回 {'id':'test_id', '@click':'onClick'}
      //     rawAttrsMap: {}, // {id: {name: 'id', value: 'test_id',start,end}, '@click': {name: '@click', value: 'onClick',start,end}}
      //     parent: parent,
      //     children: []
      //   }
      // }
      // makeAttrsMap(attrs)校验是属性是否是重复定义
      // if (map[attrs[i].name] && !isIE && !isEdge) {warn$2('duplicate attribute: ' + attrs[i].name, attrs[i]);}
      ...
      {
        if (options.outputSourceRange) { // outputSourceRange: "development" !== 'production', true
          ...
          // {id: {name: 'id', value: 'test_id'}, '@click': {name: '@click', value: 'onClick'}}
        }
        attrs.forEach(function (attr) {
          // var invalidAttributeRE = /[\s"'<>\/=]/; 属性名称中不能包含spaces, quotes, <, >, / or =.符号，否则无效
      ...
      // 此分支解析的 tag 不能是 style script text/javascript 标签，且非服务器渲染时
      if (isForbiddenTag(element) && !isServerRendering()) {
        element.forbidden = true;
        warn$2(
          // 模板应该只负责将状态映射到的UI。避免在模板中放置带有副作用的标签，例如 style script 标签它们不会被解析。
      ...
      // 当 tag = input 标签时，需要对其绑定的部分指令进行转换 
...
function parseHTML (html, options) {
  ...
  while (html) {
    ...
    // 保证if 分支处理的是非纯文本的标签内容，即不是 script 或 style
    // isPlainTextElement(tag) 函数是用 makeMap 返回的闭包结果 function (tag) { return map[tag.toLowerCase()]; }
    if (!lastTag || !isPlainTextElement(lastTag)) {
      // 如果是标签肯定是以 < 开头的，如果不是，则作为文本处理
      ...
      if (textEnd === 0) {
        ...
        */
        /**
         * handleStartTag(startTagMatch); 主要逻辑是将 match中的attrs属性值转化 [{name: 'id', value: "test-di", start, end},{name: "@click", value: "onClick"}]。
         * 然后调用 options.start 处理
         */
      ...
      // 处理文本中段含有<，要判断 < 是不是文本的一部分，还是标签开头的部分
      var text = (void 0), rest = (void 0), next = (void 0);
      if (textEnd >= 0) {
        rest = html.slice(textEnd);
        while (
          !endTag.test(rest) && // 不是结束标签中的 <
          !startTagOpen.test(rest) && // 不是开始标签中的 <
          !comment.test(rest) && // 不是注释标签中的 <
          !conditionalComment.test(rest) // 不是条件注释中的 <
        ) {
          // < in plain text, be forgiving and treat it as text
          // 那此时的 < 是作为文本中的一部分内容，那些再找下一个 < 位置，循环处理
          next = rest.indexOf('<', 1);
          if (next < 0) { break }
          textEnd += next; // 之前的< 作为文本一部分，那再加上下一个 < 的序号位置
          rest = html.slice(textEnd); // 用上面最新的 textEnd 截取后剩余部分再执行 while 判断
        }
        text = html.substring(0, textEnd); // 非开头的 < 之前的字符串作为文本处理
      }
  ...
  function handleStartTag (match) {
    ...
    if (expectHTML) { // baseOptions.expectHTML 配置为 true
      // isNonPhrasingTag 可以自闭合的标签，如 address base aside 等，在 p9270定义
      if (lastTag === 'p' && isNonPhrasingTag(tagName)) {
        parseEndTag(lastTag);
      }
      // canBeLeftOpenTag$$1 开放标签，即开标签，makeMap('colgroup,dd,dt,li,options,p,td,tfoot,th,thead,tr,source');
    ...
    // 这里是对 match.attrs 的处理，拿到属性名和属性值
    /**
    ...
    */
    ...
    for (var i = 0; i < l; i++) {
      ...
      // 如果是 a 标签，且 href 属性的值需要对值进行 decode 编码
...
function processElement ( element, options) {
  ...
  // 无格式的相素元素，即没有绑定key,没有插槽，没有绑定任何属性，如 <div>this is plain element</div>
  element.plain = (
    !element.key &&
    !element.scopedSlots &&
...
// 从 element.attrsList 获取对应name的值，并将其从 attrsList 中删除
// attrsList 是在 createASTElement(tag, attrs, currentParent) 中建立的，值等于 attrs 的映射
// attrsList = {'id':'test_id', '@click':'onClick', 'v-for': "(item,index) in list"}
function getAndRemoveAttr (
 ...
 function processFor (el) {
  ...
  if ((exp = getAndRemoveAttr(el, 'v-for'))) {
     ...
     */
    if (res) {
      extend(el, res); // 将 for / alias / iterator1 赋值给 el
    } else {
      warn$2(
...
// exp = "(item, index) in list" 或者 ”item of list"
// var forAliasRE = /([\s\S]*?)\s+(?:in|of)\s+([\s\S]*)/;
// var forIteratorRE = /,([^,\}\]]*)(?:,([^,\}\]]*))?$/;
// var stripParensRE = /^\(|\)$/g;

/**
 * "(item, index) in list".match(forAliasRE)
 * ["(item, index) in list", "(item, index)", "list", index: 0, input: "(item, index) in list", groups: undefined]
 */
function parseFor (exp) {
  ...
  var alias = inMatch[1].trim().replace(stripParensRE, ''); // item, index
  var iteratorMatch = alias.match(forIteratorRE);
  // "item, index".match(forIteratorRE)
  // [", index", " index", undefined, index: 4, input: "item, index", groups: undefined]
  if (iteratorMatch) { // 有 index 情形
    res.alias = alias.replace(forIteratorRE, '').trim(); // item
    res.iterator1 = iteratorMatch[1].trim(); // index
    if (iteratorMatch[2]) { // 循环对象的对象 (value, key, index) in object
      res.iterator2 = iteratorMatch[2].trim();
    }
  } else {
    // 对应 item in list，此时 alias = item
    res.alias = alias;
  }
  return res
...
function processRef (el) {
  ...
  if (ref) {
    el.ref = ref;
    el.refInFor = checkInFor(el); // 如果父元素存在 v-for 绑定，则为true，即refInFor字面意思：当前 ref 是在 for 循环内
  }
}
...
// 关于 element 中属性的保存的字段有 el.attrsList / el.attrsMap / el.attrs / el.dynamicAttrs / el.rawAttrsMap
...
function optimize (root, options) { // root=ast, options=baseOptions
  if (!root) { return }

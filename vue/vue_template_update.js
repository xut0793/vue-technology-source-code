...
function createPatchFunction (backend) {
  ...
  function createComponent (vnode, insertedVnodeQueue, parentElm, refElm) {
    ...
    if (isDef(i)) {
      ...
      if (isDef(i = i.hook) && isDef(i = i.init)) {
        ...
        // 这样在 createComponent 函数内就完成了组件实例的初始化和元素挂载
      ...
      // 然后在这一步，将组件元素插入到父组件中完成挂载 insert。
  ...
  function initComponent (vnode, insertedVnodeQueue) {
    ...
    if (isPatchable(vnode)) {
      // 组件实例创建完成后，调用 create 钩子函数更新组件内部所有属性、事件、指令等
      // updateAttrs / updateClass(oldVnode, vnode) / updateDOMListeners(oldVnode, vnode) / updateDOMProps(oldVnode, vnode) / updateStyle(oldVnode, vnode) / updateDirectives(oldVnode, vnode)
      invokeCreateHooks(vnode, insertedVnodeQueue);
      setScope(vnode);
    } else {
  ...
  /**
   * 设置 CSS scopeId  <div data-v-59683909></div>
   * function setStyleScope (node, scopeId) {
      node.setAttribute(scopeId, '');
    }
   */
  /**
   * 只有在 SFC 单文件组件中才可能有 <style scoped></style> 的写法
   * 而此类 .vue 文件在工程中都有 vue-loader 处理的
   * 所以 _scopedId 是在 vue-loader 完成模块编译时生成的.
   * 在 vue-loader 中调用 ./runtime/componentNormalizer 中的函数 normalizer 函数,会 vue-loader 解析后文件的路径中获取 ?id=xxx 的值传入
   * {hasScoped ? JSON.stringify(id) : `null`},
   * 然后在normalizer中:options._scopeId = 'data-v-' + scopeId
   * 参考链接:https://segmentfault.com/a/1190000039677087
   */
  ...
  return function patch (oldVnode, vnode, hydrating, removeOnly) {
    // 1. 如果新 vnode 是 undefined,然后旧 oldVnode 是存在的,则说明当前更新是删除了节点,执行卸载钩子 invokeDestoryHook
    if (isUndef(vnode)) {
    ...
    // 2. 如果旧节点不存在,新节点存在,则说明当前更新是新增节点,执行createElm
    // 如果直接执行 vm.$mount(),没有存入el,也是执行此处步骤
    if (isUndef(oldVnode)) {
       ...
       */
      // 3. 如果旧节点不是一个真实元素,是一个vitrual node,并且与新节点是同一个,则进行差异比较 patchVnode

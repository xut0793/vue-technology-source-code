...
(function (global, factory) {
  ...
  var Observer = function Observer (value) {
    ...
    if (Array.isArray(value)) {
      // this.dep = new Dep() // 如果只是单纯添加数组依赖，可以只在这里定义。但现实还用于 watch 中 deep=true时定义。
      if (hasProto) {
   ...
   */
  var strats = config.optionMergeStrategies; // 空对象 {} = Object.create(null)
  ...
  /*  */
  //#region initEvents $on $once $off $emit
  /*  */
  ...
  var Watcher = function Watcher (
    ...
    if (options) {
      ...
      this.user = !!options.user; // 用户在option.watch自定义的 watcher，即 user-watch,
  ...
  Watcher.prototype.cleanupDeps = function cleanupDeps () {
    ...
    while (i--) {
      var dep = this.deps[i];
      if (!this.newDepIds.has(dep.id)) { // 上一轮收集的依赖在当前这轮依赖收集后不存在的，直接删除
        dep.removeSub(this);
      }
    }
    // 将最新一轮收集到的 newDepIds 给 depIds, newDeps 给 Deps，后 newDepIds/newDeps 清空
    var tmp = this.depIds;
  ...
  function initMixin (Vue) {
    Vue.prototype._init = function (options) {
      ...
      if (options && options._isComponent) {
        ...
        /**
         * createComponent => i => componentVNodeHooks.init => createComponentInstanceForVnode 调用
         * var options = {
         *  _isComponent: true,
         *  _parentVnode: vnode,
         *  parent: parent
        ...
        if (isDef(children)) {
          ...
          if (!elm.hasChildNodes()) {
            ...
            if (isDef(i = data) && isDef(i = i.domProps) && isDef(i = i.innerHTML)) {
              ...
              // iterate and compare children lists 递归比较子列表
  ...
  /**
   * var baseModules = [ ref, directives ]; // line: 6719
   */
  ...
  //#endregion transition 动画组件
  /*  */

  //#region  transition-group 动画组件
  var props = extend({
    tag: String,
    moveClass: String
  }, transitionProps);
  ...
  //#endregion transition-group 动画组件
  /*  */
  ...
  // (and which close themselves) // 必须有内容的标签
  ...
  function parse (
    ...
    delimiters = options.delimiters; // 自定义分隔符，默认是 {{ }}
  ...
  function processAttrs (el) {
    ...
    for (i = 0, l = list.length; i < l; i++) {
      ...
      value = list[i].value;
      if (dirRE.test(name)) { // dirRE = /^v-|^@|^:|^#/; v-on / @ / v-bind / : / v-slot / #
  ...
  function parseModifiers (name) {
    // var modifierRE = /\.[^.\]]+(?=[^\]]*$)/g;
    // 'click.native.stop'.match(modifierRE) => [".native", ".stop"]
  ...
  // attrs=[{name: 'id', value: 'test_id'}, {name: '@click', value: 'onClick'}]
